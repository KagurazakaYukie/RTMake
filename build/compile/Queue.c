#include "../include/compile/Queue.h"
#include <stdbool.h>
#include <semaphore.h>
#include <stdio.h>
...
QueueManager *QueueManagerInit(MTMemoryManager *mm) {
    ...
    MemoryInfo *mi = MTMemoryManagerUnitCalloc(mbu, sizeof(QueueManager));
    MemoryInfo *semm = MTMemoryManagerUnitCalloc(mbu, sizeof(sem_t));
...
Queue *QueueManagerAdd(MTMemoryManager *mm, QueueManager *qm, void *b, void *data, void *note) {
    MemoryInfo *mi = MTMemoryManagerUnitCalloc(qm->mbu, sizeof(Queue));
    Queue *q = (Queue *) mi->m;
    q->mi = mi;
    q->data = data;
    q->ft = b;
...
void QueueManagerDestroy(MTMemoryManager *mm, QueueManager *qm) {
    ...
    sem_destroy(qm->sem);
    MTMemoryManagerAppointInitUnit(qm->mbu);
}
